...
import { submitWritingGrade, aiGradeWriting } from '../lib/supabaseService';
import {
  ArrowLeft,
  Loader2,
  CheckCircle,
...
interface WritingGradeDetailProps {
  submission: any;
  onBack: () => void;
  onGraded: () => void;
}

export const WritingGradeDetail: React.FC<WritingGradeDetailProps> = ({
  submission,
  onBack,
  onGraded,
}) => {
  ...
  const [criteria, setCriteria] = useState<Record<string, boolean>>({
    grammar: submission.grade?.criteria_scores?.grammar ? true : false,
    ...
    task_achievement: submission.grade?.criteria_scores?.task_achievement ? true : false,
  });
  ...
  const handleAiGrading = async () => {
    setAiLoading(true);
    try {
      const result = await aiGradeWriting(
        submission.exams?.prompt || 'Writing Task',
        submission.content,
      ...
      if (result.criteriaScores) {
        setCriteria({
          grammar: result.criteriaScores.grammar >= 5,
          vocabulary: result.criteriaScores.vocabulary >= 5,
          coherence: result.criteriaScores.coherence >= 5,
          task_achievement: result.criteriaScores.task_achievement >= 5,
        });
      }
    } catch (err: any) {
      console.error('AI Grading failed:', err);
      setSubmitError('AI Grading failed. Please grade manually.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleSubmitGrade = async () => {
    ...
    try {
      await submitWritingGrade(
        submission.id,
        submission.user_id, // graderId - using current user context if available
        score,
        feedback,
        {
          grammar: criteria.grammar ? 1 : 0,
          vocabulary: criteria.vocabulary ? 1 : 0,
          coherence: criteria.coherence ? 1 : 0,
          task_achievement: criteria.task_achievement ? 1 : 0,
        }
      );
      ...
      <header className="bg-gradient-to-r from-indigo-900 to-indigo-700 text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <BookMarked size={24} className="text-emerald-300" />
            <div>
              <h1 className="text-xl font-bold">Grade Writing</h1>
              <p className="text-indigo-200 text-sm">
                {submission.exam_title} - Task {submission.task_id}
              </p>
            </div>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg dark:shadow-gray-900/30 p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-indigo-900 dark:text-gray-100">{submission.user_name}</h3>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg dark:shadow-gray-900/30 overflow-hidden">
              <div className="bg-gradient-to-r from-amber-500 to-orange-600 px-6 py-3 text-white">
              ...
              <div className="p-6">
                <p className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed whitespace-pre-line">
                  {submission.exams?.prompt || 'Writing task prompt not available'}
                </p>
              </div>
            </div>
            ...
            {/* Student Response */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg dark:shadow-gray-900/30 overflow-hidden">
              <div className="bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-3 text-white">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold flex items-center gap-2">
                    <BookMarked size={18} />
                    Student Response
                  </h3>
                  <span className="text-sm text-indigo-200">{wordCount} words</span>
                </div>
              </div>
              <div className="p-6">
                <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4 text-gray-800 dark:text-gray-200 whitespace-pre-wrap min-h-[200px] border border-gray-200 dark:border-gray-600 text-sm leading-relaxed">
                  {submission.content || (
          ...
          <div className="space-y-6">
            {/* Score Input */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg dark:shadow-gray-900/30 p-6">
              <h3 className="font-bold text-indigo-900 dark:text-gray-100 mb-4 flex items-center gap-2">
            ...
            {/* Criteria Checkboxes */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg dark:shadow-gray-900/30 p-6">
              <h3 className="font-bold text-indigo-900 dark:text-gray-100 mb-4">Criteria Assessment</h3>
              <div className="space-y-3">
                {[
                  ...
                  <label
                    key={criterion.key}
                    className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-gray-600 hover:border-indigo-300 hover:bg-indigo-50/50 dark:hover:bg-gray-700 transition-all cursor-pointer"
                  >
                    <input
                      type="checkbox"
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg dark:shadow-gray-900/30 p-6">
              ...
              <textarea
                ...
                rows={6}
                className="w-full p-4 border-2 border-gray-200 dark:border-gray-600 rounded-xl resize-y focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all text-sm text-gray-700 dark:text-gray-200 placeholder-gray-400 dark:bg-gray-700"
              />
            </div>
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-purple-500 to-purple-700 text-white rounded-xl font-semibold hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {aiLoading ? (
                  <>
              ...
              <button
                ...
                disabled={submitting || aiLoading || submitSuccess}
